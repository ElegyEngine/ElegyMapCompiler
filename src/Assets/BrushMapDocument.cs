...
using Elegy.MapCompiler.Data.Map;
using Elegy.Text;
...
namespace Elegy.MapCompiler.Assets
{
	public class BrushMapDocument
	{
		...
		public static Entity ParseEntity( Lexer lex )
		{
			...
			{
				Console.WriteLine( $"WARNING: Entity does not have a classname! {lex.GetLineInfo()}" );
		...
		public static BrushMapDocument? FromValve220MapFile( string path )
		{
			...
			{
				Console.WriteLine( $"Cannot find map file '{path}', oops" );
				return null;
			}

			Console.WriteLine( $"Loading map '{path}'..." );
			...
			{
				Console.WriteLine( $"Error while parsing .map: {exception.Message}" );
